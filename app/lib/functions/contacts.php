...
function showContactList(){
    ...
    $contacts_empty = "<p id='list-empty'>It's kind of lonely here</p>";
    ...
    if($contacts_raw){
        foreach ($contacts_raw as $contact_id => $contact){
            ...
            $contact = "
           <li $data_contact_id $data_fav class='list-item'>
...
function getContactDetails($contact_id){
    ...
    if($contact){
        ...
        if(!empty($contact['image'])){
            $contact['image-html'] = "
            <img id='contact-image' src='avatars/$image' width='64' height='64' alt='' srcset='avatars/$image 2x'/>
